...
for key, frequency_params in frequencies.items():
    ...
    for num in range(4):
        ...
        picks_epochs = mne.pick_types(
            preprocessing.epochs.info, meg="mag", exclude="bads"
        )
    ...
    for acq_idx, epoch in enumerate(freq_epochs):
        ...
        epoch.drop_bad()
        epoch.pick(picked_channels)

    all_epochs = mne.concatenate_epochs(freq_epochs)
    file_name = (
        DATA_SAVE + "gestures_epochs/freq_bands_bad_channels/" + key + "_all_epochs.pkl"
    )
    ...
    file_name = (
        DATA_SAVE
        + "gestures_epochs/freq_bands_bad_channels/"
        + key
        + "_all_epochs_decimated.pkl"
    )
    with open(file_name, "wb") as f:
        pickle.dump(all_epochs, f)
